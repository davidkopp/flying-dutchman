 ...
 * -----
 * Last Modified: Sunday, 7th March 2022
...
(function ($, exports)
{
    $(document).ready(function ()
    {
        ...
        let user = DatabaseAPI.Users.getUserDetailsByUserName(userDetails.username);
        let main_menu_items = "";
        let ordered_items = "";
        let bar_inventory = DatabaseAPI.Inventory.getInventory(Constants.INVENTORIES.BAR);
        let vip_inventory = DatabaseAPI.Inventory.getInventory(Constants.INVENTORIES.VIP);
        let bar_items = "";
        let vip_items = "";

        for (let i = 0; i < bar_inventory.length; i++)
        {
            beverage = DatabaseAPI.Beverages.findBeverageByNr(bar_inventory[i].beverageNr);
            bar_items = bar_items +
            `
        ...
        {
            beverage = DatabaseAPI.Beverages.findBeverageByNr(vip_inventory[i].beverageNr);
            vip_items = vip_items +
            `
        ...
        $("#vip-account").append("Weclome "+user.first_name+" "+user.last_name+", Balance = "+user.creditSEK);
