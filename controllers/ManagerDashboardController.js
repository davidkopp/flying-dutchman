...
(function ($, exports)
{
    $(document).ready(function ()
    {
        ...
        $("#get-amount-button").click(function()
        {
            ...
            if(serialNumber !== '')
            {
                for (let index = 0; index < BeveragesDB.beverages.length; index++)
                {
                    ...
                    if(beverage.nr == serialNumber)
                    {
                        ...
                        $("#amount").empty();
                        $("#amount").append(`<h6>${beverage.priceinclvat}</h6>`);
                        break;
                    }
                }
            }
            ...
            {
                $("#amount").empty();
                console.log('ManagerDashboardController | Search with a valid serial number');
            }
        });
        ...
        $("#revise-amount-button").click(function()
        {
            ...
            if(finalindex != 0 && beverage != null && newvalue != '')
            {
                ...
                console.log(`Price of '${beverage.name}' changed to '${newvalue}'`);
                alert(`Price of '${beverage.name}' changed to '${newvalue}'`);
            }
            else
            {
                console.log(`Value of Index = '${finalindex}', Value of Beverage = '${beverage.name}', New Value = '${newvalue}'`)
                finalindex = 0;
                beverage = null;
            }
        });
        ...
        $("#get-quantity-button").click(function()
        {
            ...
            inventoryItem = DatabaseAPI.Inventory.getInventory();
            if(serialNumber !== '')
            {
        ...
        $("#refil-beverages-button").click(function()
        {
            ...
            if(finalindex != 0 && beverage != null && newvalue != '')
            {
                let item = DatabaseAPI.Inventory.updateNumberInStockForBeverage(beverage.beverageNr, parseInt(newvalue)+parseInt(stocks));
                console.log(`'${newvalue}' stocks ordered for '${beverage.beverageNr}'`);
                alert(`'${newvalue}' stocks ordered for '${beverage.beverageNr}'`);
            }
            else
            {
                console.log(`Value of Index = '${finalindex}', Value of Beverage = '${beverage.name}', New Value = '${newvalue}'`)
        ...
        function initMenu()
        {
            ...
            for (let i = 0; i < inventoryItems.length; i++)
            {
                ...
                active = DatabaseAPI.ActiveCheck.getStatusOfBeverage(inventoryItem.beverageNr)
                active == true ? active = "Active" : active = "Removed";
                // console.log("Status"+active);
                beverage = DatabaseAPI.Beverages.findBeverageByNr(inventoryItem.beverageNr);
                menu = menu +
                `
                    <span id="span">
                        <span id="span-`+inventoryItem.beverageNr+`">
                            ${inventoryItem.beverageNr}
                            ${beverage.name}
                            ${active}
                        </span>
                        <button id="beverage-`+inventoryItem.beverageNr+`-button" class="beverage-show-hide-button">Show/Hide</button>
                    </span>
                    <br>
                `;
            }
